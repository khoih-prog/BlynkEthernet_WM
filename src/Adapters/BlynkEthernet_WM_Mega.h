...
<!DOCTYPE html><html><head><title>BlynkEthernet_WM</title><style>div,input{padding:2px;font-size:1em;}input{width:95%;}\
...
#define RFC952_HOSTNAME_MAXLEN      24
    ...
    void setRFC952_hostname(const char* iHostname = "")
    {
      if (iHostname[0] == 0)
      {
        String _hostname = "Mega-W5X00-XXXXXX";    // + String(macAddress, HEX);
...
#error EPROM_START + CONFIG_DATA_SIZE > EEPROM_SIZE. Please adjust.
    ...
    bool getConfigData()
    {
           ...
           (calChecksum != BlynkEthernet_WM_config.checkSum) )
      {
        ...
        BLYNK_LOG2(F("InitEEPROM,sz="), EEPROM_SIZE);
