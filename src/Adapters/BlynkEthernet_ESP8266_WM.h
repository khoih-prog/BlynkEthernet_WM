...
// -- HTML page fragments
const char BLYNK_WM_HTML_HEAD[]     /*PROGMEM*/ = "<!DOCTYPE html><html><head><title>BlynkEthernet_ESP8266_WM</title><style>div,input{padding:2px;font-size:1em;}input{width:95%;}\
...
#define  CONFIG_PORTAL_FILENAME_BACKUP    BLYNK_F("/wm_cp.bak")
  ...
  // Stating positon to store BlynkEthernet_WM_config
      ...
      if (server)
      {
        ...
        if (key == "" && value == "")
        {
          ...
          if ( RFC952_hostname[0] != 0 )
          {
            ...
            result.replace("BlynkEthernet_ESP8266_WM", RFC952_hostname);
          }
          else if ( BlynkEthernet_WM_config.board_name[0] != 0 )
          {
            // Or replace only if board_name is valid.  Otherwise, keep intact
            result.replace("BlynkEthernet_ESP8266_WM", BlynkEthernet_WM_config.board_name);
          }
